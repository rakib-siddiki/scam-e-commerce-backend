...
const loginController = async (req, res) => {
    try {
        ...
        if (!existingUser)  return res.status(400).send({ message: "User with this email doesn't exist" });
        const isMatch = await bcrypt.compare(password,existingUser.password)
        if (!isMatch)  return res.status(400).send({ message: "Password is incorrect" });
        if(!existingUser.verify) return res.status(400).send({ message: "Please verify your email" });
